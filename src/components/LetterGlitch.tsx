...
const LetterGlitch = ({
  ...
  const drawLetters = () => {
    ...
    // Optional blue→pink gradient across the canvas width
    let gradient: CanvasGradient | null = null;
    if (useGradient) {
      gradient = ctx.createLinearGradient(0, 0, width, 0);
      const stops = gradientStops && gradientStops.length > 0 ? gradientStops : ['#60a5fa', '#a855f7', '#ec4899'];
    ...
    letters.current.forEach((letter, index) => {
      ...
      ctx.fillStyle = useGradient && gradient ? gradient : letter.color;
